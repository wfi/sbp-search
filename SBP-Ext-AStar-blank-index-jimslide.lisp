...
(let
((scaled-h (* **h-scale** (t-piece-h-fun new-blanks-index **intermediate-position**))))
((succ-h (+ (h-error scaled-h) scaled-h)))
...
;;; h-error
(defun h-error (h)
(cond ((!= (- h *h_1*) 0) (progn (setf *h_1* h)
(setf *global-error* (/ (+ (* *global-error* *steps_1*)
(- h *steps*))
*steps*))
(setf *steps_1* steps)
(setf *steps* 0)
*global-error*))
(else (progn (setf *steps* (+ *steps* 1))
*global-error*))))
